...
if [ "$OS_LINX" != "" ]
then
    cp -fr ./libs/wxWidgets/* /usr/lib32/
    cp -fr ./libs/ace/* /usr/lib32/
    cp -fr ./libs/opswat/* /usr/lib32/
    cp -fr ./libs/libInode* /usr/lib32/
    if [ ! -r "/usr/lib32/libtiff.so.3" ]
    then
        cp -fr ./libs/std/libtiff.so.3 /usr/lib32/libtiff.so.3
    fi
	...
	rm -f /usr/lib/libACE-5.6.1.so 
	rm -f /usr/lib/libACEXML-5.6.1.so 
	rm -f /usr/lib/libACEXML_Parser-5.6.1.so 
...
then
    ...
    if [ ! -r "/usr/lib/libtiff.so.3" ] && [ ! -r "/usr/lib/i386-linux-gnu/libtiff.so.3" ]
    then
        cp -fr ./libs/std/libtiff.so.3 /usr/lib/libtiff.so.3
    fi
    if [ ! -r "/usr/lib/i386-linux-gnu/libjpeg.so.62" ] && [ ! -r "/usr/lib/libjpeg.so.62" ]
    then
